<div style="width: 100%;height: 100%;background: url(images/product_new.png); background-size: 100% 100%;">
  <div style="height:75%;">
    <div style="width:35%; padding: 40px 40px">
      <img src="images/logo.png" alt="logo" />
    </div>
    <div style="font-size: 28px; font-weight: bold; color:#000;text-align: center; margin-bottom: 60px;">
      性能强、稳定性好、接口丰富的 ARM 工控机
    </div>
  </div>
  <div style="padding-left: 40px;">
    <div style="font-size: 40px; font-weight: bold; color:#000;margin-bottom: 30px;">
      InBOX732 系列
    </div>
    <div style="text-align: center;">
      <div style="display: flex; flex-wrap: wrap; gap: 16px; ">
        <div style="width: 200px;background-color: #4CAF50; color: white; padding: 8px 8px; border-radius: 6px; font-size: 18px;">· 4G/WiFi/蓝牙</div>
        <div style="width: 200px;background-color: #4CAF50; color: white; padding: 8px 8px; border-radius: 6px; font-size: 18px;">· 六核处理器</div>
      </div>
      <div style="display: flex; flex-wrap: wrap; gap: 16px;margin-top:16px">
        <div style="width: 200px;background-color: #4CAF50; color: white; padding: 8px 8px; border-radius: 6px; font-size: 18px;">· 丰富接口</div>
        <div style="width: 200px;background-color: #4CAF50; color: white; padding: 8px 8px; border-radius: 6px; font-size: 18px; ">· 工业级设计</div>
      </div>
    </div>
  </div>
</div>

<div style="page-break-after: always;"></div>

# <span style="color: green;">1. 产品概述</span>

**InBOX732 系列 ARM 工控机面向工商业领域，提供强劲配置与丰富接口的数据整合处理与设备联网能力。**

**产品特点：**

- **强劲配置:** 六核处理器，2GB/16GB存储，4K输出，接口丰富
- **多系统支持:** 可选Android/Linux系统，深度优化长期稳定
- **无缝网络:** 支持4G/WiFi/有线连接，保障设备通讯不间断
- **工业级设计:** 金属外壳IP40，Level3级EMC，-20°C~70°C宽温
- **丰富接口:** 4路RS232、2路RS485、5路USB、HDMI、双网口等

## <span style="color: green;">核心技术指标</span>

| 项目             | 规格                                         |
| -------------- | ------------------------------------------ |
| 蜂窝网络           | 4G 全网通                                     |
| Wi-Fi          | 802.11a/b/g/n，Client / AP 模式               |
| 蓝牙             | 蓝牙 5.0                                     |
| 操作系统           | Linux(Debian9) / Android10                 |
| 视频输出           | HDMI2.0，支持 4K@60fps                        |
| 网络接入           | 4G / WiFi / 有线                             |
| 尺寸 (W × D × H) | 190.2 × 160.2 × 43.6 mm                    |
| 安装方式           | 壁挂式安装                                      |
| 接口             | 4×RS232、2×RS485、5×USB、2×千兆网口、HDMI、CAN、GPIO |
| 供电             | 9–24 V DC                                  |
| 工作温度           | -20 °C ~ +70 °C                            |
| 防护等级           | IP40                                       |

# <span style="color: green;">2. 产品尺寸</span>

<div style="text-align: center; margin-bottom: 16px;">
  <img src="images/prod_4.png" width="80%" alt="产品尺寸图" />
</div>

<div>
  <div>注意：</div>
  <div>1.所有尺寸单位为毫米（mm）。</div>
  <div>2.所有尺寸均为近似值，<span style="font-weight: bold;">仅供参考</span>。</div>
  <div>3.图示尺寸<span style="font-weight: bold;">不得用于生产加工</span>。</div>
  <div>4.尺寸需符合零件及制造公差要求。</div>
  <div>5.尺寸如有变更，恕不另行通知。</div>
</div>

# <span style="color: green;">3. 硬件规格</span>

| 类别/参数                                        | 规格                                         |
| -------------------------------------------- | ------------------------------------------ |
| <span style="color: green;">**处理器**</span>   |                                            |
| CPU                                          | 瑞芯微六核处理器，最高主频 1.8 GHz                      |
| 内存                                           | 2 GB                                       |
| 存储                                           | 16 GB eMMC                                 |
| <span style="color: green;">**连接与联网**</span> |                                            |
| 蜂窝网络                                         | 4G 全网通                                     |
| SIM 卡规格                                      | 1.8 V / 3 V，抽屉式卡座 × 1                      |
| SD 卡槽                                        | SD 卡槽 × 1                                  |
| 天线接口                                         | 3G/4G: SMA × 1；Wi-Fi: RP-SMA × 2           |
| <span style="color: green;">**接口**</span>    |                                            |
| 以太网                                          | 2 × 10/100/1000 Mbps，LAN/WAN               |
| 串口                                           | 4 × RS232 (DB9 公头)；2 × RS485 (3 pin 端子)    |
| USB                                          | 4 × USB 2.0；1 × USB 3.0                    |
| HDMI                                         | HDMI 2.0 × 1，支持 4K@60fps                   |
| CAN                                          | CAN × 1                                    |
| GPIO                                         | 10 pin 绿色端子 × 1 (IN1 ~ 4、GND、OUT1 ~ 4、GND) |
| 音频                                           | MIC 3.5 mm × 1；SPK 双声道 4 Ω 3 W × 1         |
| 调试接口                                         | Type-C × 1                                 |
| 按键                                           | 电源键(Power) × 1；模式键(Mode) × 1               |
| <span style="color: green;">**无线**</span>    |                                            |
| Wi-Fi                                        | 802.11a/b/g/n，Client / AP 模式               |
| 蓝牙                                           | 蓝牙 5.0                                     |
| <span style="color: green;">**指示灯**</span>   |                                            |
| 指示灯                                          | 电源、状态、Wi-Fi、3G/4G                          |
| <span style="color: green;">**电源**</span>    |                                            |
| 输入电源                                         | 9–24 V DC                                  |
| 功耗                                           | 整机小于 12 W（不带外设）                            |
| <span style="color: green;">**机械规格**</span>  |                                            |
| 尺寸 (W × D × H)                               | 190.2 × 160.2 × 43.6 mm（含安装件）              |
| 安装方式                                         | 壁挂式安装                                      |
| 防护等级                                         | IP40                                       |
| 散热                                           | 无风扇散热                                      |
| 外壳工艺                                         | 金属                                         |
| <span style="color: green;">**环境**</span>    |                                            |
| 工作温度                                         | -20 °C ~ +70 °C                            |
| 储存温度                                         | -40 °C ~ +85 °C                            |
| 湿度                                           | 5 ~ 95 % RH（无凝霜）                           |
| <span style="color: green;">**电磁兼容**</span>  |                                            |
| EMC 指标                                       | 静电 Level 3；EFT Level 3；浪涌 Level 3          |

# <span style="color: green;">4. 软件规格</span>

| 类别/参数                                       | 规格                                 |
| ------------------------------------------- | ---------------------------------- |
| <span style="color: green;">**操作系统**</span> |                                    |
| 操作系统                                        | Linux(Debian9) / Android10         |
| <span style="color: green;">**网络特性**</span> |                                    |
| 网络制式                                        | 4G 全网通                             |
| Wi-Fi                                       | 802.11a/b/g/n，支持 Client / AP 模式    |
| 蓝牙                                          | 蓝牙 5.0                             |
| <span style="color: green;">**多媒体**</span>  |                                    |
| 图形处理                                        | 双 ISP 800 MPix/s，支持双路摄像头、3D、深度信息提取 |
| 视频编解码                                       | H.265/H.264/VP9 4K@60fps 高清解码      |
| 图像格式                                        | BMP, JPG, PNG, GIF                 |
| <span style="color: green;">**配置管理**</span> |                                    |
| 定时开关机                                       | 支持                                 |
| 系统升级                                        | 本地 USB 升级                          |

# <span style="color: green;">5. 订购信息</span>

## <span style="color: green;">型号规则</span>

**Model code:** InBOX732-\<WMNN\>-\<STD/PLAT/L\>-\<A\>-\<S\>
\<WMNN\>: Cellular Type & Module（蜂窝类型与模块）
\<STD/PLAT/L\>: OS
\<A\>: —
\<S\>: Serial port type

## <span style="color: green;">产品型号</span>

| 型号              | 国家   | \<WMNN\>: Cellular Networks                                                                                                                                                                                            | \<STD/PLAT/L\>: OS | \<A\> | \<S\>: Serial port type |
| --------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |:-----:|:-----------------------:|
| InBOX732-DQ20-L | 中国   | LTE-FDD: B1/B3/B5/B8<br/>LTE-TDD: B34/B38/B39/B40/B41<br/>WCDMA: B1/B8<br/>TD-SCDMA: B34/B39<br/>CDMA/EVDO: BC0<br/>GSM/EDGE: 900/1800 MHz                                                                             | Debian             | —     | —                       |
| InBOX732-FQ58-L | EMEA | LTE-FDD: B1/B3/B7/B8/B20/B28A<br/>WCDMA: B1/B8<br/>GSM/EDGE: B3/B8                                                                                                                                                     | Debian             | —     | —                       |
| InBOX732-FQ39-L | 北美   | LTE-FDD: B2/B4/B5/B7/B12/B13/B25/B26/B29/B30/B66 2×CA<br/>B2+B2/B5/B12/B13/B29; B4+B4/B5/B12/B13/B29;<br/>B7+B5/B7/B12/B26;<br/>B25+B5/B12/B25/B26;<br/>B30+B5/B12/B29;<br/>B66+B5/B12/B13/B29/B66<br/>WCDMA: B2/B4/B5 | Debian             | —     | —                       |
| InBOX732-EN00-L | —    | —                                                                                                                                                                                                                      | Debian             | —     | —                       |

# <span style="color: green;">6. 联系我们</span>

- **官网：** [映翰通官网](https://www.inhand.com.cn)
- **版权声明：** ©映翰通网络 保留所有权利
